...
import { Weapon, Character } from '@/lib/data';
...
function WeaponDetailContent({ weapon }: WeaponDetailClientProps) {
  ...
  const recommendedCharacters = weapon.recommendedCharacters 
    ? characters.filter(char => weapon.recommendedCharacters!.includes(char.slug.current))
    : [];
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <nav className="flex items-center space-x-2 text-sm">
            ...
            <span className="text-gray-400">/</span>
            <span className="text-gray-900 font-medium">{t(weapon.name)}</span>
          </nav>
        </div>
      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {recommendedCharacters.map((character) => (
                        <Link
                          ...
                          className="bg-gray-50 p-4 rounded-lg border border-gray-200 hover:bg-gray-100:bg-gray-600 transition-colors"
                        >
